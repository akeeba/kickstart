...
class AKS3Request
{
     ...
     * @param   AKS3Configuration  $configuration  The Amazon S3 configuration object to use
     */
    function __construct($verb, $bucket = '', $uri = '', AKS3Configuration $configuration)
    {
        $this->verb          = $verb;
        $this->bucket        = $bucket;
        $this->uri           = '/';
        ...
        $this->response = new AKS3Response();
    }
     ...
     *
     * @param   AKS3Input  $input
     *
     * @return  void
     */
    public function setInput(AKS3Input $input)
    {
        $this->input = $input;
    }
    ...
    public function getAuthenticatedURL($lifetime = null, $https = false)
    {
        ...
        $signer = AKS3Signature::getSignatureObject($this, $this->configuration->getSignatureMethod());
    ...
    public function getResponse()
    {
        ...
        switch ($this->verb)
        {
            ...
            case 'PUT':
            case 'POST':
                if (!is_object($this->input) || !($this->input instanceof AKS3Input))
                {
                    $this->input = new AKS3Input();
                }

                $size = $this->input->getSize();
                $type = $this->input->getInputType();

                if ($type == AKS3Input::INPUT_DATA)
                {
        ...
        {
            $this->response->error = new AKS3Error(
                curl_errno($curl),
                curl_error($curl),
                $this->resource
            );
     ...
     * where applicable."
     *
     * @param   AKS3Configuration  $configuration
     * @param   string         $bucket
     *
     * @return  string
     */
    private function getHostName(AKS3Configuration $configuration, $bucket)
    {
        //
